            ...
            <div class="modal-wrapper">
                <div class="modal-container">
                    <div class="modal-header">
                        <h3 v-if="gubun == 1"><i class="fa fa-clipboard-list"></i> 시스템변경사항등록</h3>
                        <h3 v-else><i class="fa fa-clipboard-check"></i> 시스템변경사항수정</h3>
                    </div>
...
export default {
    ...
    methods: {
        insertStory: function() {
            ...
            this.$EventBus.$emit("changeInsertStory", this.storyinfo);
        },
        updateStory: function() {
            // this.storyinfo.writer = this.$store.state.user.username ;
            this.storyinfo.comp_no = this.comp.value;
            this.storyinfo.comp_name = this.comp.text;
            this.$EventBus.$emit("changeUpdateStory", this.storyinfo);
        },
        deleteStory: function() {
            this.$EventBus.$emit("changeDeleteStory", this.storyinfo);
        },
        closeStory: function() {
            this.$EventBus.$emit("changeCloseStory", this.storyinfo);
        }
    }
};
</script>
